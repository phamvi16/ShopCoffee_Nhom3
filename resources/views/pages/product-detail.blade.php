            ...
            <div class="pp-right-column col-sm-7 col-md-7">
                ...
                <div class="product-information">
                    <div id="short-description">
                        <p><i>{!! Str::limit($pro->Description, 80, '... <a href="#description">More</a>') !!}</i></p>
                    </div>
                    <div class="product-actions">
                        <div class="product-variants">
                            <div class="clearfix product-variants-item d-flex align-items-center mt-5 mb-1">
                                <span class="control-label">Size: </span>
                                <ul id="product-size-area">
                                    @foreach ($pro->product_size->sortByDesc('Size') as $size)
                                        @csrf
                                        <li class="input-container pull-xs-left">
                                            <input class="input-radio input-size" type="radio" name="Size"
                                                data-product-id="{{ $size->Id_Product ?? '' }}"
                                                data-buyprice="{{ $size->Sale_Price }}"
                                                data-saleprice="{{ number_format($size->Sale_Price, 0, '.', '.') ?? 0 }}"
                                                data-price="{{ number_format($size->Price, 0, '.', '.') ?? 0 }}"
                                                value="{{ $size->Id ?? '' }}" {{ $loop->first ? 'checked' : '' }}>
                                            <span class="radio-label input-size">{{ $size->Size ?? '' }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <div class="product-add-to-cart">
                            <div class="add">
                                @if ($pro->Visibility == 'Publish')
                                    <a href="#" data-id="{{ $size->Id_Product }}" name="add-to-cart" type="add-to-cart"
                                        data-toggle="modal" data-target="#exampleModal" data-pos="prodetail"
                                        class="btn btn-primary add-to-cart mr-4">THÊM GIỎ HÀNG</a>
